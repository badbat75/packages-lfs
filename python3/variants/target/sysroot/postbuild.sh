...
sed -e 's/-fuse-ld=.* / /g' \
	...
	-i "${PKG_PKGPATH}${PYTHON_LIBDEST}"/_sysconfigdata__*.py
### make install byte-compiles the whole standard library through the interpreter of the build: every .pyc
### records the staging directory (co_filename), so the standard library ships as .py only
find "${PKG_PKGPATH}${PYTHON_LIBDEST}" -name '__pycache__' -type d -prune -exec rm -rfv {} +
grep -r -l -F --include='*.pyc' -e "${PKG_PKGPATH}/" "${PKG_PKGPATH}${PYTHON_LIBDEST}" | xargs -r rm -v
...
PYTHON_RECORDS=("${PKG_PKGPATH}${PYTHON_LIBDEST}"/_sysconfig{data__*.py,_vars__*.json} "${PKG_PKGPATH}${PYTHON_LIBPL}"/{Makefile,config.c})
